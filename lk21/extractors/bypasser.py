...
class Bypass(BaseExtractor):
    ...
    def bypass_letsupload(self, url):
        ...
        if (fileId := re.search(r"(?i)showFileInformation\((\d+)\)", raw.text)):
            return self.bypass_redirect(
                'https://letsupload.io/account/direct_download/' + fileId[1]
            )
    ...
    def bypass_zippyshare(self, url):
        ...
        if (dlbutton := re.search(r'href = "([^"]+)" \+ \(([^)]+)\) \+ "([^"]+)', raw.text)):
            ...
            return f'{re.search(r"https?://[^/]+", raw.url)[0]}{folder}{math_chall}{filename}'
    ...
    def bypass_fembed(self, url):
        ...
        if api is not None:
            result = {}
            raw = self.session.post("https://layarkacaxxi.icu" + api[1]).json()
            for d in raw["data"]:
                f = d["file"]
